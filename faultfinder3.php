...
$wholedata = file('sanhw1.txt');
$wholedata = array_map('trim',$wholedata);
for ($i=0;$i<count($wholedata);$i++)
{
    $explodedata[$i] = explode(':',$wholedata[$i]);
    $worddata[$i] = $explodedata[$i][0];
    $dictdata[$i] = $explodedata[$i][1];
}
...
// This is the place we will have to change for different dictionaries. first parameter is the base dictionary. second is fixed. Third is the dictionary we want to check.
    comparepatterns($filea,$pattern,$fileb,$outfile); 
}
 ...
 */
function comparepatterns($filea,$pattern,$filec,$outfile)
{
