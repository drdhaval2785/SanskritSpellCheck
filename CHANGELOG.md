...
- **The 22 zero-fileable dicts do NOT need a second run — scope question closed (H2281,
  ...
  deliberately left this as a cheap probe rather than assuming it either way. Re-ran the two
  highest-volume zero-fileable dicts: **BHS 551 candidates re-judged (15 classified TYPO, all
  refuted at source-confirm) · SCH 644 (8 classified TYPO, all refuted)** — 1,195 candidates,
  **0 confirmed**, matching both committed zeros. Same pipeline and per-phase models as D7/D9
  (Sonnet 5 `claude-sonnet-5` classify → Opus 5 `claude-opus-5` source-confirm → Opus 5 review).
  - **The zeros are structural, not stochastic**, and the two dicts fail differently. **BHS**
    carries its own critical apparatus and has already labelled its misprints
    (`[laṭikā, app. misprint for latikA (so Index)]`, `[Anantaryasamādhi, misprint in Mvy 901 …
    corrected in Index]`) — the detector re-finds what Edgerton annotated, and "correcting" it
    would destroy the apparatus. **SCH** corroborates the headword in its own body (`girī` a
    distinct cvi-adverb, `samadhurA` a feminine, `yudda` = *yud+da* "battle-giving") — the
    minimal-pair suggestion is simply wrong. Neither is a coin-flip a third run might land
    differently, which is what separates this from the SHS/YAT/ACC instability in H9.
  - New `corrections_draft/REFUTED_TYPO_CANDIDATES_PROBE22.tsv` (23 rows): every candidate that
    reached the source-confirm gate and died there, with the classify reason and the Opus
    refutation quoting the entry. On a zero-fileable dictionary the union table is empty and
    carries no information — this is the informative residue, and per H3 the do-not-file
    evidence is the durable deliverable. Generated by `corrections_draft/dump_refuted_typos.py`.
  - **Stated limit:** 2 of 22 tested, chosen as the highest-volume and therefore likeliest to
    yield. The other 20 are smaller and mostly the same mature/foreign-gloss/specialist-index
    kinds; extending the sweep is not supported by this evidence.
...
- **A scan vote on a net-new row could not be applied — it had no queue row to land in
  ...
  The rows verified by [H2274](https://github.com/drdhaval2785/SanskritSpellCheck/pull/68) existed
  only in the former, so approving one would have been reported `missing` and lost. A 182-row
  sheet whose votes land for only 109 rows is worse than a 109-row sheet, because the loss is
  invisible at voting time. `corrections_draft/append_netnew_to_queues.py` appends them
  **append-only** (existing rows never rewritten, reordered or re-flagged — the same
  "union, never overwrite" guardrail the union passes exist to honour), all with the trailing
  `:n`, so nothing becomes fileable without a human scan vote. **56 rows appended** across
  SHS +31 · YAT +18 · PWG +4 · WIL +2 · GST +1. Live queue rows: SHS 37→68, YAT 27→45,
  PWG 12→16, WIL 3→5, GST 1→2.
  - The **5 ACC EDITORIAL rows are deliberately excluded** — the corrected spelling already
    exists as its own `<k1>`, so they are headword-collision decisions for an editor, not plain
    corrections. That is the entire point of the class.
### Added
- **`corrections_draft/REVIEWED_OUT_VS_UNION_CONFLICTS.md` — 17 rows two passes disagree about.**
  Run-1's Opus false-positive review ruled each **not fileable** (they sit commented out in the
  queue); the union passes re-found them and H2274's verification ruled them fileable against the
  entry text. **Nothing was uncommented**: a prior review decision must not be reversed by a
  silent flip, so both judgements are recorded, the rows appear in the sheet carrying the newer
  verdict, and a human's scan vote is the arbiter. **16 of the 17 are YAT b/v pairs** — the class
  YAT's own package already marked "held for scan", i.e. exactly where the scan is decisive.
  Generated by `corrections_draft/report_reviewed_out_conflicts.py`.
