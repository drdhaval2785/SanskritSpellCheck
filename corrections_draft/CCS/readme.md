...
- **[CCS_wrong_readings.txt](CCS_wrong_readings.txt)** — the standing **do-not-file** list (3).
- **[CCS_triaged.txt](CCS_triaged.txt)** — the full six-bucket queue.
- **[CCS_file_first_sf.txt](CCS_file_first_sf.txt)** — empty of fileable rows.

## Method & provenance

Driver `python triage_dict.py CCS`, German `triage_lang` profile (`CCS → de`), hybrid model
split. Raw engine output (do NOT apply): [CCS_candidates.txt](CCS_candidates.txt),
[CCS_draft.txt](CCS_draft.txt) — superseded by the triage.
