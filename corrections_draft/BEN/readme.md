...
- **[BEN_wrong_readings.txt](BEN_wrong_readings.txt)** — the standing **do-not-file** list (14).
- **[BEN_triaged.txt](BEN_triaged.txt)** — the full six-bucket queue.
- **[BEN_file_first_sf.txt](BEN_file_first_sf.txt)** — empty of fileable rows.

## Method & provenance

Driver `python triage_dict.py BEN`, English `triage_lang` profile, hybrid model split. Raw
engine output (do NOT apply): [BEN_candidates.txt](BEN_candidates.txt),
[BEN_draft.txt](BEN_draft.txt) — superseded by the triage.
