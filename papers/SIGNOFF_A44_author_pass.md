...
6. **Relative links that die outside the repo tree.** §3.2 `detectors/combined_candidates.txt` points at a file §4.3 says is gitignored; §4.6 `../corrections_draft/irr/`, Appendix A `../detectors/` and `../corrections_draft/` are relative paths. URLs are a hard limit for this pass; they need full blob URLs (or removal) before the anonymised pack is built.
