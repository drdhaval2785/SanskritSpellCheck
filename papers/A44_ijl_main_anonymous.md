...
docs/PRIOR_ART.md (Supplementary File S24)). **(b) LLM-assisted correction
...
toolset's headword spine `sanhw1.txt` (Supplementary File S2). Of the dictionaries in that
...
(`detectors/get_external_source.py` (Supplementary File S3)); it was
...
`detectors/combined_candidates.txt` (Supplementary File S1)) are
...
`/dict-triage <DICT>` (Supplementary File S12) skill (operational detail
...
1. **Deterministic pre-settlement** (no model). Language-specific markers
   (`detectors/triage_lang.py` (Supplementary File S4)) settle the declared
...
harness: `detectors/eval.py` (Supplementary File S8) checks that the suppress list
...
`detectors/gen_do_not_file_suppress.py` (Supplementary File S6)
into `nochange/do_not_file_suppress.txt` (Supplementary File S7), which
`detectors/slp1util.py` (Supplementary File S5) `load_whitelist()` unions with the
...
a public umbrella issue in the CDSL `CORRECTIONS` repository *(reference withheld for anonymous review)*), not
applied here.
...
headwords** (`nochange/do_not_file_suppress.txt` (Supplementary File S7)).
...
(`corrections_draft/SHS/readme.md` (Supplementary File S15); each class
is deployed with full in-entry evidence lines in the live umbrella issue
a public umbrella issue in the CDSL `CORRECTIONS` repository *(reference withheld for anonymous review)*):
...
`corrections_draft/file_first_verified.tsv` (Supplementary File S17),
narrative in `corrections_draft/VERIFICATION_2026_07.md` (Supplementary File S16)):
...
(`corrections_draft/irr/` (Supplementary File S18); inputs built by
`irr_build_inputs.py` (Supplementary File S19), agreement computed with
exact rational arithmetic by `irr_agreement.py` (Supplementary File S20)).
...
`agreement_stats.md` (Supplementary File S21)). On the
...
(`detectors/irr_cross_family.py` (Supplementary File S22) re-runs the
...
held-out gold set (`detectors/gold_corrections.tsv` (Supplementary File S9),
derived from `file_first_verified.tsv` (Supplementary File S17)):
...
rows that must **not** be silently "corrected". `detectors/eval.py` (Supplementary File S8)
...
- **The reliability number is model-vs-model, not human IRR.** §4.6 reports a blind
  ...
  `corrections_draft/irr/HUMAN_ANCHOR_NEEDED.md` (Supplementary File S23).
...
`/dict-triage <DICT>` (Supplementary File S12) skill over
`detectors/triage_*.py` (Supplementary File S11) and
`detectors/bodyaware_workflow.js` (Supplementary File S10). Each run
...
sections); see `corrections_draft/<DICT>/` (Supplementary File S13). The 2026-07-02
...
`corrections_draft/README.md` (Supplementary File S14); the re-verification
record in `corrections_draft/VERIFICATION_2026_07.md` (Supplementary File S16)
and `file_first_verified.tsv` (Supplementary File S17). The
deduplicated suppression artifact is
`nochange/do_not_file_suppress.txt` (Supplementary File S7),
regenerated with `cd detectors && python gen_do_not_file_suppress.py`; its safety
harness is `detectors/eval.py` (Supplementary File S8) (0 false positives vs ~31k
...
`detectors/gold_corrections.tsv` (Supplementary File S9). To re-verify the
...
- Prasanna, S. (2022). Spellchecker for Sanskrit: The Road Less Taken. *Proceedings
  ...
  *(Verified 10-07-2026 by the H452 prior-art scan; replaces the earlier "ISCLS 2024
  ...
  See docs/PRIOR_ART.md (Supplementary File S24).)*
